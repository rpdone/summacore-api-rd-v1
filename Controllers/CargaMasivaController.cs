...
namespace SummaCore.Controllers
{
    ...
    public class CargaMasivaController : ControllerBase
    {
        ...
        public CargaMasivaController(ILogger<CargaMasivaController> logger, DgiiService dgiiSender, IConfiguration configuration)
        {
            _logger = logger;
        ...
        {
            if (archivoCsv == null || archivoCsv.Length == 0) return BadRequest("Por favor sube un archivo CSV o Excel válido.");
            ...
            try
            {
                ...
                List<CsvEcfDto> registros = (extension == ".xlsx" || extension == ".xls") ? LeerExcel(archivoCsv) : await LeerCsv(archivoCsv);
                ...
                foreach (var dto in registros)
                {
                    ...
                    try
                    {
                        ...
                        // IMPORTANTE: Crear Signature vacía para pasar validación XSD (requiere tag <Signature>)
                        ecf.Signature = new Signature(); 
                        
                        var response = await _dgiiSender.Facturar(ecf, certificate);
                        bool success = response.Contains("trackId") || response.Contains("TrackId");
                        object respJson = response;
                        try { respJson = System.Text.Json.JsonSerializer.Deserialize<object>(response); } catch { }
                        reporte.Add(new { linea = lineaNum, encf = dto.Encf, estado = success ? "ENVIADO" : "RECHAZADO", respuesta = respJson });
                    }
                    catch (Exception ex)
                    {
                        reporte.Add(new { linea = lineaNum, encf = dto.Encf, estado = "ERROR_VALIDACION", error = ex.Message });
                        _logger.LogError(ex, $"Error en línea {lineaNum}: {dto.Encf}");
                    }
                }
        ...
        {
            var ecf = new ECF();
            ecf.Encabezado.Version.Value = "1.0";
            ...
            if (dto.TipoeCF == 33 || dto.TipoeCF == 34)
            {
                if (dto.TipoeCF == 34) ecf.Encabezado.IdDoc.IndicadorNotaCredito = 1; // CORREGIDO: 1 es válido, 2 inválido
                ecf.InformacionReferencia = new InformacionReferencia { NCFModificado = "E310000000001", FechaNCFModificado = DateTime.Now.ToString("dd-MM-yyyy"), CodigoModificacion = 1 };
            }
            ...
            if (dto.MontoGravadoI1 > 0) {
                ...
                ecf.DetallesItems.Item.Add(CreateItem(lineNum++, "Bienes al 18%", 1, dto.MontoGravadoI1, 1, dto.TipoeCF));
            }
            if (dto.MontoGravadoI2 > 0) {
                ...
                ecf.DetallesItems.Item.Add(CreateItem(lineNum++, "Bienes al 16%", 2, dto.MontoGravadoI2, 2, dto.TipoeCF));
            }
            if (dto.MontoExento > 0) {
                ecf.DetallesItems.Item.Add(CreateItem(lineNum++, "Bienes Exentos", 4, dto.MontoExento, 0, dto.TipoeCF));
            }
            ...
            ecf.Subtotales.Subtotal.Add(new Subtotal
            {
                ...
                SubTotalITBIS = dto.TotalITBIS > 0 ? dto.TotalITBIS : null,
                SubTotalITBIS1 = dto.TotalITBIS1 > 0 ? dto.TotalITBIS1 : null,
                SubTotalITBIS2 = dto.TotalITBIS2 > 0 ? dto.TotalITBIS2 : null,
        ...
        private Item CreateItem(int lineNumber, string name, int indicator, decimal amount, int taxType, int tipoeCF)
        {
            var item = new Item
            {
            ...
            if (tipoeCF == 41) item.Retencion = new RetencionItem { IndicadorAgenteRetencionoPercepcion = 1, MontoISRRetenido = 0.00m };
            if (tipoeCF == 47) item.Retencion = new RetencionItem { IndicadorAgenteRetencionoPercepcion = 1, MontoISRRetenido = Math.Round(amount * 0.27m, 2) };
        ...
        private async Task<List<CsvEcfDto>> LeerCsv(IFormFile archivo) { return new List<CsvEcfDto>(); }
    }
}